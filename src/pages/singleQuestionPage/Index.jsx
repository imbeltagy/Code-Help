...
const SingleQuestionPage = () => {
  ...
  const { username, isLogged, isFetching } = useSelector((state) => state.user);
  ...
  useEffect(() => {
    const getQuestions = async () => {
      ...
      const res = await fetchApi(
        `get_questions?question_id=${[questionID].join("&question_id=")}&current_user_username=${
          isLogged ? username : "none"
        }`,
        "GET"
      );
      if (res.success) {
        ...
        const questionData = {
          ...
          date: new Date(question.publish_date).getTime(),
          isSolved: question.solved_state,
          isSaved: false,
        };
        dispatch(pushQuestion({ id: questionID, data: questionData }));
